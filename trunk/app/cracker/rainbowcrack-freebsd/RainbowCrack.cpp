...
void Usage()
{
	...
	printf("       rcrack rainbow_table_pathname -a hash hash hash <up to 20>\n");
	...
	printf("         rcrack *.rt -f hash.txt\n");
 	printf("   	 rcrack *.rt -a 5d41402abc4b2a76 b9719d911017c592\n");
}
...
int main(int argc, char* argv[])
{
	...
	char *additionalInput[20];
	int i = 0;
	for(i = 0; i < 20 && i < argc - 4; i++)
	{
		additionalInput[i] = argv[i + 4];
	}
	...
	string sInputType        = argv[6];
	string sInput            = argv[7];
	char *additionalInput[20];
	int i, length;
        for(i = 0; i < 20 && i < argc - 7; i++)
        {
                additionalInput[i] = argv[i + 7];
        }
	...
	else if (sInputType == "-a")
	{
		fCrackerType = true;

		int i;
		for (i = 0; (additionalInput[i] != '\0') && (i < 20) && (i < length); i++)
		{
			string sHash = additionalInput[i];
			if (NormalizeHash(sHash))
				vHash.push_back(sHash);
			else
				printf("invalid hash: %s\n", sHash.c_str());
	...
	else if (sInputType == "-l")   // password list
	{
		fCrackerType = true;
